...
TrinitoWindow::TrinitoWindow(QWidget *parent) : QWidget(parent) {
  ...
  setStyleSheet(
    ...
    "QListWidget { background-color: #090f20; border: 1px solid #1e293b; border-radius: 8px; padding: 5px; outline: 0; }"
...
QWidget *TrinitoWindow::createManageTab(const QString &packType, const QString &displayName) {
  ...
  // Asignar la lista a la variable miembro correspondiente para usarla en loadPacks
...
void TrinitoWindow::loadPacks(const QString &packType, QListWidget *listWidget) {
  ...
  if (!packDir.exists()) {
    // QMessageBox::information(this, "Info", "No se encontraron " + packType + ".");
    return; // Si no existe la carpeta, salir
  }

  QStringList entries = packDir.entryList(QDir::Dirs | QDir::Files | QDir::NoDotAndDotDot);
  ...
  // Conectar la señal itemChanged del QListWidget para detectar cambios en los checkboxes
  connect(listWidget, &QListWidget::itemChanged, this, [=](QListWidgetItem *changedItem) {
    ...
    if (newState) {
      // Si está habilitado y el nombre termina en .disabled, removerlo
      if (packName.endsWith(".disabled")) {
        newName = packName.chopped(9); // .disabled.length() = 9
      }
    } else {
      // Si está deshabilitado y el nombre no termina en .disabled, añadirlo
      if (!packName.endsWith(".disabled")) {
        newName = packName + ".disabled";
      }
    }
    // Actualizar el texto del item (esto puede causar un nuevo itemChanged si no se maneja cuidadosamente)
    // Para evitar loops, podrías comparar newName con el texto actual antes de cambiarlo
...
void TrinitoWindow::togglePack(const QString &packType, const QString &packName, bool enable) {
  QString baseDataDir = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation)
  + "/mcpelauncher/games/com.mojang";
        QString packDirPath = baseDataDir + "/" + packType;
        ...
        if (enable) {
          ...
          if (packName.endsWith(".disabled")) {
            ...
            if (process.exitCode() == 0) {
              ...
              // QMessageBox::information(this, "Éxito", QString("Pack '%1' habilitado.").arg(packName));
            } else {
              QString err = process.readAllStandardError();
              QMessageBox::critical(this, "Error", QString("No se pudo habilitar el pack '%1'.\nError al descomprimir:\n%2").arg(packName).arg(err.isEmpty() ? "Error desconocido." : err));
              return;
            }
          }
          ...
          if (!packName.endsWith(".disabled")) {
            ...
            // Comando: tar -czf <archivo_salida> -C <directorio_padre> <nombre_carpeta_o_archivo>
            // Ejemplo: tar -czf pack1.mcpack.disabled.tar.gz -C /ruta/contenedora pack1.mcpack
            QFileInfo fileInfo(oldPath);
            QString parentDir = fileInfo.absolutePath(); // Directorio padre del pack
            QString baseName = fileInfo.fileName();      // Nombre del pack (archivo o carpeta)
            ...
            if (process.exitCode() == 0) {
              ...
              // QMessageBox::information(this, "Éxito", QString("Pack '%1' deshabilitado.").arg(packName));
            } else {
              QString err = process.readAllStandardError();
              QMessageBox::critical(this, "Error", QString("No se pudo deshabilitar el pack '%1'.\nError al comprimir:\n%2").arg(packName).arg(err.isEmpty() ? "Error desconocido." : err));
              return;
            }
          }
        ...
        // Opcional: Actualizar la lista de packs en la UI si estás en la pestaña correspondiente
        // Por ejemplo, si estás en la pestaña "Gestionar Mods":
        // if (currentTab == "Gestionar Mods") {
        //     loadPacks("behavior_packs", modsList);
        // }
        // O simplemente mostrar un mensaje de éxito general aquí si prefieres no recargar constantemente.
}

// ... resto del código (createPackTab, createDevTab, createWorldTab, installItem) ...
...
QWidget *TrinitoWindow::createPackTab(const QString &targetSubdir, const QString &labelText) {
  ...
  connect(deleteButton, &QPushButton::clicked, this, [=]() {
    if (listWidget->selectedItems().isEmpty()) {
      QMessageBox::warning(this, "Advertencia", "No hay ningún elemento seleccionado.");
      return;
    }
    ...
    int r = QMessageBox::warning(
      this, "Advertencia",
      QString("¿Estás seguro de eliminar '%1'?\nEsta acción no se puede deshacer.").arg(selectedEntry),
    ...
    QString baseDataDir = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation)
    + "/mcpelauncher/games/com.mojang";
      QString fullPath = baseDataDir + "/" + targetSubdir + "/" + selectedEntry;
      ...
      if (success) {
        QMessageBox::information(this, "Éxito", QString("'%1' eliminado correctamente.").arg(selectedEntry));
        // Recargar la lista
        loadPacks(targetSubdir, listWidget);
      } else {
        QMessageBox::critical(this, "Error", QString("No se pudo eliminar '%1'.").arg(selectedEntry));
      }
  });
...
QWidget *TrinitoWindow::createDevTab() {
  ...
  QPushButton *deleteBehButton = new QPushButton("Eliminar Behavior Pack Seleccionado");
  connect(deleteBehButton, &QPushButton::clicked, this, [=]() {
    if (behListWidget->selectedItems().isEmpty()) {
      QMessageBox::warning(this, "Advertencia", "No hay ningún Behavior Pack seleccionado.");
      return;
    }
    ...
    QString baseDataDir = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation)
    + "/mcpelauncher/games/com.mojang";
      QString fullPath = baseDataDir + "/development_behavior_packs/" + selectedEntry;
      ...
      if (success) {
        QMessageBox::information(this, "Éxito", QString("Behavior Pack '%1' eliminado correctamente.").arg(selectedEntry));
        // Recargar la lista
        loadPacks("development_behavior_packs", behListWidget);
      } else {
        QMessageBox::critical(this, "Error", QString("No se pudo eliminar el Behavior Pack '%1'.").arg(selectedEntry));
      }
  });
  deleteLayout->addWidget(deleteBehButton);

  // Botón para eliminar un pack seleccionado en la lista de Resource Packs
  QPushButton *deleteResButton = new QPushButton("Eliminar Resource Pack Seleccionado");
  connect(deleteResButton, &QPushButton::clicked, this, [=]() {
    if (resListWidget->selectedItems().isEmpty()) {
      QMessageBox::warning(this, "Advertencia", "No hay ningún Resource Pack seleccionado.");
      return;
    }
    ...
    QString baseDataDir = QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation)
    + "/mcpelauncher/games/com.mojang";
      QString fullPath = baseDataDir + "/development_resource_packs/" + selectedEntry;
      ...
      if (success) {
        QMessageBox::information(this, "Éxito", QString("Resource Pack '%1' eliminado correctamente.").arg(selectedEntry));
        // Recargar la lista
        loadPacks("development_resource_packs", resListWidget);
      } else {
        QMessageBox::critical(this, "Error", QString("No se pudo eliminar el Resource Pack '%1'.").arg(selectedEntry));
      }
  });
...
QWidget *TrinitoWindow::createWorldTab() {
  ...
  connect(deleteButton, &QPushButton::clicked, this, [=]() {
    if (listWidget->selectedItems().isEmpty()) {
      QMessageBox::warning(this, "Advertencia", "No hay ningún mundo seleccionado.");
      return;
    }
    ...
    int r = QMessageBox::warning(
      this, "Advertencia",
      QString("¿Estás seguro de eliminar el mundo '%1'?\nEsta acción no se puede deshacer.").arg(selectedWorld),
                                 QMessageBox::Yes | QMessageBox::No, QMessageBox::No
    );
    if (r == QMessageBox::No) return;
    ...
    + "/mcpelauncher/games/com.mojang";
      ...
      if (QDir(worldPath).removeRecursively()) {
        QMessageBox::information(this, "Éxito", "Mundo eliminado correctamente.");
        // Actualizar la lista
        loadPacks("minecraftWorlds", listWidget);
      } else {
        QMessageBox::critical(this, "Error", "No se pudo eliminar el mundo.");
      }
  });
  layout->addWidget(deleteButton);
